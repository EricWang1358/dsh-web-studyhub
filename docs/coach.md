...
- **答错**后，陪学会指出一个你最可能没弄懂的点，配一个具体例子和一道二选一小检查。点「懂了」或「还是不懂」回复，不用打字。「还是不懂」最多会换两个角度再讲，之后给出「去对话里细问」。
- 同一次答错只生成一个陪学点。服务端在记录答案时就开始生成，面板请求时直接复用这次结果。答对不调用模型。
- 第一次答错时会征求意见：要不要在后台备题（变式题、应用场景题）。选过之后，陪学会问一次学习目标（考试 / 面试 / 工作中落地 / 兴趣），用来调整题目的场景。
...
- 开启备题后，答错的题、标记太简单/太难的题，以及只练了概念的一轮，都会成为变式题的目标。目标先攒批（每批最多 4 个、最多等 20 秒），一次调用写完，每张卡单独过代码校验，不合格的直接丢弃。最多同时备 12 道。
- 每轮结束时的小结先由代码算出认知层次分布（记忆 / 概念辨析 / 应用分析）、正确率、薄弱主题和反馈标签。例如一轮里七成以上是记忆/概念辨析题，正确率又高，就判定为「看似会了，但还没练过放进真实场景的应用分析」，并为答对的概念题准备应用题。至少答了 3 道时，模型只负责写一句建议、选下一步（白名单动作），并更新画像摘要。
