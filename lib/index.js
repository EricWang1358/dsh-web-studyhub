...
export function modelCompletion(ctx, route, sessionId, { light = false } = {}) {
  const complete = async (system, prompt, timeoutMs = 180000, externalSignal, maxTokens, task = "light") => {
    ...
    try {
      ...
      if (light && ended - started >= 2000)
        console.warn(`[study-model] ${task} ${succeeded ? "ok" : "failed"}: config ${Math.round(configured - started)}ms, first text ${firstText === null ? "none" : Math.round(firstText - configured) + "ms"}, generation ${firstText === null ? 0 : Math.round(ended - firstText)}ms, total ${Math.round(ended - started)}ms`);
    }
  };
  if (light) return (system, prompt, options = {}) => complete(system, prompt, 90000, undefined, options.maxTokens, options.task);
