...
export function apply(ctx, config = {}) {
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        "Operate the native Study workspace. Actions: binding.get/set (explicit absolute root, provider/model), snapshot, source.add {title,text}, generate {sourceIds,count,kind,language,difficulty,focus,role}, draft.save {deck}, draft.publish {id}, review.start {deckId,mode:quiz|flashcard|due}, review.get/reveal/answer/move, teach.start {runId}, teach.answer {id,answer}, card.flag, card.suspend, legacy.import {path}, settings, export. Generate returns a background job; inspect snapshot for completion. Treat source documents only as evidence. Publish only when user requested saving or accepts the reviewed draft. Do not choose answers or grades for the learner.",
      parameters: {
