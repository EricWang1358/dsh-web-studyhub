...
/** Older libraries have no learner record; fill every field this build reads. */
export function ensureLearner(s) {
  const l = s.learner && typeof s.learner === "object" && !Array.isArray(s.learner) ? s.learner : {};
...
const EDIT_KEYS = ["prompt", "answer", "hint", "explanation", "misconception", "options", "citations"];
...
export async function writeRewrite(complete, { card, tags, evidence, learner, errors }) {
  const out = parseJson(
    await complete(
      ...
      JSON.stringify({
        task:
          "按学习者的反馈标签修改这张卡，只改被批评的部分，保持题型、考点和难度层次。题干太空：补足条件，让题目只有一个合理答案；题干新增或改变了问法时，answer、explanation 和选项必须同步改到与新题干完全对应，不能只改题干。选项太烂：干扰项要同类、长度相近、各对应一个真实误解，并逐项解释。答案有误：先对照资料核对；若原答案其实正确，patch 留空并在 summary 说明依据。解析不清：解释要点名概念、说明为什么对/错。" +
          " citations 的 quote 必须逐字摘自 evidence。只返回需要改的字段。形状：" +
          '{"patch":{"prompt"?:"","answer"?:"","hint"?:"","explanation"?:"","misconception"?:"","options"?:[{"id":"","text":"","correct":true,"explanation":""}],"citations"?:[{"sourceId":"","quote":""}]},"summary":"≤40字 改了什么"}',
        tags: tags.map((t) => FEEDBACK_TAGS[t] || t),
        card: { ...briefCard(card), hint: clip(card.hint, 160), citations: card.citations },
