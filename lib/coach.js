...
export async function writeRewrite(complete, { card, tags, evidence, learner, errors }) {
  const out = parseJson(
    await complete(
      ...
      JSON.stringify({
        task:
          "按学习者的反馈标签修改这张卡，只改被批评的部分，保持题型、考点和难度层次。题干太空：补足条件，让题目只有一个合理答案。选项太烂：干扰项要同类、长度相近、各对应一个真实误解，并逐项解释。答案有误：先对照资料核对；若原答案其实正确，patch 留空并在 summary 说明依据。解析不清：解释要点名概念、说明为什么对/错。" +
