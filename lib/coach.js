...
export function learnerAnswer(card, feedback, { selfGraded = false } = {}) {
  ...
  if (Number.isInteger(feedback.grade)) return { text: `自评 ${feedback.grade} 分`, expected: clip(card.answer, 60) };
  return null;
}
...
export async function writeNudge(complete, { card, answer, earlier = [], learner }) {
  const out = parseJson(
    await complete(
      ...
      JSON.stringify({
        task:
          "学习者刚答错这道题。先看 learnerAnswer 和 correctAnswer：判断学习者为什么会写出/选出这个答案（例如名字相近、把 A 当成 B、只记住了一半），指出这个具体的混淆点（不要与 earlier 重复），explain 里点名学习者的答案，用一个具体例子讲清两者的区别。再出一道两选一小检查，只能有一个没有争议的正确项，考的正是这个混淆点。" +
