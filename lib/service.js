...
import {
  ...
  cognitiveLevel, debriefRules, ensureLearner, evidenceWindows, learnerAnswer, runMetrics, threadView, trimLogs,
...
function searchTerms(input) {
  ...
  if (!terms.length) throw new Error("Give a query with at least one term of 2+ characters");
  return terms;
}
...
const HANDLERS = {
    ...
    "source.search": async function (a) {
      const terms = searchTerms(a.query ?? a.terms);
      const s = await this.store.read();
