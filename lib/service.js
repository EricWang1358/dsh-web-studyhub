  ...
  for (const run of s.runs.filter((r) => !r.closedAt)) {
    ...
    for (const entry of run.entries)
      if ((entry.deckId ?? run.deckId) === deck.id && entry.card.id === card.id) {
        ...
        // later copies of the card (tail retries) still get the new version.
...
function staleReviewEntries(s, run) {
  ...
  return run.entries.flatMap((entry) => {
    if (entry.keepSnapshot) return [];
    const live = liveCard(s, run, entry);
    return live && contentKey(live) !== contentKey(entry.card) ? [{ entry, live }] : [];
  });
}
...
const HANDLERS = {
    "review.get": async function (a) {
      ...
      return this.store.update((latest) => {
        ...
        let reset = false;
        for (const { entry, live } of staleReviewEntries(latest, current)) reset = syncReviewEntry(entry, live) || reset;
    ...
    "snapshot": async function (a) {
      ...
      // Polling panels send their last fingerprint; skip building and sending
      // a full snapshot (sources included) when nothing it shows can differ.
      // Due counts move with time, so the fingerprint rolls over each minute.
      const fingerprint = JSON.stringify([
        s.revision, this.store.root, !!this.complete, Math.floor(Date.now() / 60000), coach,
        [...jobs.values()].filter((j) => j.root === this.store.root).map(publicJob),
      ]);
...
export class StudyService {
  ...
  coachTask(kind, label, meta, work) {
    ...
    const previous = coachQueues.get(root) || Promise.resolve();
  ...
  flushPrep() {
    ...
    if (!pending?.targets.length) return coachQueues.get(root) || Promise.resolve();
    clearTimeout(pending.timer);
    prepPending.delete(root);
    let last;
    for (let i = 0; i < pending.targets.length; i += 4) {
      const batch = pending.targets.slice(i, i + 4);
      last = this.coachTask("prep", `准备 ${batch.length} 道定制题`, {}, () => this.writePrepared(batch));
    }
    return last;
  }
