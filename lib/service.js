...
const substance = (c) =>
  JSON.stringify([c.kind, c.prompt, c.answer, (c.options || []).map((o) => [o.id, o.text, o.correct])]);
/**
...
function checkScope(s, scope) {
  ...
  return scope.map((x) => {
    ...
    if (x.cardId) {
      findCard(s, x);
      return { deckId: x.deckId, cardId: String(x.cardId) };
    }
  ...
  async call(action, a = {}) {
    ...
    if (action === "capture") {
      ...
      const dependent =
        a.requiredBy === "current" || parsed.prerequisite
          ? currentCard(state)
          : a.requiredBy
            ? findCard(state, a.requiredBy) && { deckId: a.requiredBy.deckId, cardId: a.requiredBy.cardId }
            : null;
    ...
    return this.store.update((s) => {
      switch (action) {
        ...
        case "review.start": {
          if (!["quiz", "flashcard", "due", "wrong", "path"].includes(a.mode))
