...
/** Validate a scope list of {deckId, topic} or {deckId, cardId}; card refs resolve by id. */
export function checkScope(s, scope) {
  ...
  return scope.map((x) => {
    ...
    if (!s.decks.some((d) => d.id === x?.deckId))
...
export function prerequisiteView(state, deckId, card, outcome) {
  return (card.requires || []).flatMap((r) => {
    ...
    if (!pre || deck.systemKind === "slain") return [];
    return [
      {
        deckId: deck.id,
        cardId: pre.id,
