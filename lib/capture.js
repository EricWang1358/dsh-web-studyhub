 ...
 */
export async function captureQuestion(complete, state, { question, kind, related, notes = "" }) {
  ...
  const topic = String(placement.topic || "").trim().slice(0, 120) || "未分类";
  ...
  let attempt = check(parseJson(await complete(system, `${instruction}\nDATA:\n${data}`)));
