...
export function createHostHandler(ctx, config = {}, makeComplete) {
  return async function handle(_endpoint, request) {
    try {
      ...
      /* 后台助教要派子代理，只有宿主层拿得到 subagents 和父代理，所以在这里
         接住；结果由子代理写回题库，面板通过快照看任务状态。 */
      if (action === "assist.start") {
        ...
        return traced({
          ...
          value: await startAssist(ctx, {
            ...
            route: (await binding(cwd, config)).route || sessionModel(ctx, session),
          }),
        });
      }
      const value = await service.call(action, args);
